...
<c:if test="${not empty sessionScope.account}">
  <meta name="currentUserId" content="${sessionScope.account.getUserID()}"/>
</c:if>
<c:if test="${not empty requestScope.product}">
  <meta name="productId" content="${requestScope.product.getBookID()}"/>
  <meta name="productName" content="${requestScope.product.getTitle()}"/>
</c:if>
